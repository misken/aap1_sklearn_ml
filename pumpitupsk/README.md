...
For the remote repository, make a github repository named pumpitup, then do;
...
cookiecutter) or replace `pumpitup` with some other name like `someothername`.

```bash
conda env create -f aap.yml
# conda env create -f aap.yml -n pumpitup
```
